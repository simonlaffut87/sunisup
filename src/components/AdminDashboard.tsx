...
export function AdminDashboard() {
  ...
  const verifyAdminConnection = async () => {
    console.log('🔍 VÉRIFICATION CONNEXION ADMIN SUPABASE');
    ...
    try {
      ...
      console.log('✅ Utilisateur vérifié:', {
        id: user?.id,
        email: user?.email,
        isAdmin: user?.email === 'info@sunisup.be'
      });

      // 3. Tester l'accès à la table participants
      console.log('📋 Étape 3: Test d\'accès à la table participants...');
      ...
      if (accessError) {
        console.error('❌ Erreur accès participants:', {
      ...
      console.log('✅ Accès participants réussi:', testData?.length || 0, 'participants trouvés');

      // 4. Tester une opération d'écriture
      console.log('📋 Étape 4: Test d\'écriture (simulation)...');
      ...
      if (writeTestError) {
        console.warn('⚠️ Erreur test écriture:', writeTestError);
      }
      ...
      console.log('🎉 CONNEXION ADMIN VÉRIFIÉE AVEC SUCCÈS');
      toast.success(`✅ Connexion admin vérifiée pour ${user?.email}`);

    } catch (error: any) {
      console.error('❌ ERREUR GÉNÉRALE vérification connexion:', error);
      setConnectionStatus({
        isConnected: false,
        userEmail: null,
        hasValidSession: false,
