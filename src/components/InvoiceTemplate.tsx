            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="mt-8 border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-6 py-3 border-b border-gray-200">
                  <h4 className="font-semibold text-gray-900">Détail par participant - Groupe {participant.groupe}</h4>
                  <p className="text-sm text-gray-600 mt-1">
                    Données individuelles pour la période {format(new Date(selectedPeriod.startMonth + '-01'), 'MMMM yyyy', { locale: fr })}
                    {selectedPeriod.startMonth !== selectedPeriod.endMonth && 
                      ` - ${format(new Date(selectedPeriod.endMonth + '-01'), 'MMMM yyyy', { locale: fr })}`
                    }
                  </p>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {groupParticipants.map((groupParticipant) => {
                        ...
                        const individualTotals = processIndividualMonthlyData(groupParticipant, selectedPeriod);

                        const isCurrentParticipant = groupParticipant.id === participant.id;

                        return (
                          <tr key={groupParticipant.id} className={isCurrentParticipant ? 'bg-amber-50' : 'hover:bg-gray-50'}>
                              ...
                              <div className="flex items-center">
                                <div className="flex-shrink-0 w-10 h-10">
                                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                    ...
                                    {groupParticipant.type === 'producer' ? (
                                      <Sun className={`w-5 h-5 ${isCurrentParticipant ? 'text-amber-700' : 'text-amber-600'}`} />
                                    ) : (
                                      <Building2 className={`w-5 h-5 ${isCurrentParticipant ? 'text-blue-700' : 'text-blue-600'}`} />
                                    )}
                                  </div>
                                </div>
                                <div className="ml-4">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {(individualTotals.volume_partage / 1000).toFixed(3)} MWh
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {(individualTotals.volume_complementaire / 1000).toFixed(3)} MWh
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {(individualTotals.injection_partagee / 1000).toFixed(3)} MWh
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {(individualTotals.injection_complementaire / 1000).toFixed(3)} MWh
                            </td>
                          </tr>
                        );
                      })}
