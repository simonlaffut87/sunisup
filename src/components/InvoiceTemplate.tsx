...
export function InvoiceTemplate({ isOpen, onClose, participant, selectedPeriod }: InvoiceTemplateProps) {
  ...
  React.useEffect(() => {
    if (isOpen && participant && selectedPeriod) {
      calculateAndStoreBillingData();
    }
  }, [isOpen, participant, selectedPeriod]);

  const calculateAndStoreBillingData = async () => {
    try {
      ...
      months.forEach(monthKey => {
        ...
        if (monthData) {
          ...
          monthlyDetails.push({
            ...
            allColumns: monthData.allColumns || {},
      ...
      if (error) {
        ...
        toast.success('Données de facturation calculées et sauvegardées');
      }
